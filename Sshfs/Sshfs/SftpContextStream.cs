...
namespace Sshfs
{
	internal sealed class SftpContextStream : Stream
	{
		...
		/// </summary>
		private readonly int optimalReadRequestSize;
		...
		private readonly int WRITE_BUFFER_SIZE;
		...
		public SftpFileAttributes Attributes
		{
			get
			{
				lock(this)
				{
					if(!_writeMode) return _attributes;
					//FlushWriteBuffer();
					SetupRead();
		...
		private void ReadAsync(long position, int count, byte[] buffer, int bufferOffset, Action<int> Received)
		{
			...
			Debug.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + "\t" +
			                " Sending Async read for offset " + bufferOffset);
#endif
			this._session.RequestReadAsync(_handle, (ulong) position, (uint) count,
				response =>
				{
#if DEBUG
					Debug.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + "\t" + " Got data for offset " +
					                bufferOffset);
#endif
		...
		private int getDataFromBuffer(long position, int count, byte[] dst, int dstOffset, ref bool isEOF)
		{
			if((position < this.readBufferPosition) ||
			   (position >= this.readBufferPosition + this.readBufferCount)) return 0;
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			else //writeBuffer space insufficient
			{
				...
				if(count > WRITE_BUFFER_SIZE) //writeBuffer size is still lower
				{
					...
					while(remainingcount >= WRITE_BUFFER_SIZE) //fire whole blocks
					{
						...
						Buffer.BlockCopy(buffer, offset + suboffset, _writeBuffer, _writeBufferPosition /*always zero*/,
							chunkcount);
						_session.RequestWrite(_handle, (ulong) (_position + suboffset), _writeBuffer, 0, chunkcount,
							null);
					...
					{
						Buffer.BlockCopy(buffer, offset + suboffset, _writeBuffer, _writeBufferPosition /*shoud be 0*/,
							remainingcount);
			...
			_session.RequestWrite(_handle, (ulong) (_position - WRITE_BUFFER_SIZE), _writeBuffer, 0,
				_writeBufferPosition, null);
		...
		private void FlushWriteBuffer()
		{
			...
			_session.RequestWrite(_handle, (ulong) (_position - _writeBufferPosition), data, 0, _writeBufferPosition,
				null);
