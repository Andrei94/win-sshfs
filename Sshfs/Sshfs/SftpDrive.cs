		...
		private void SetupFilesystem()
		{
			...
			switch(ProxyType)
			{
				case 1:
					...
					break;
				case 2:
					pt = ProxyTypes.Socks4;
					break;
				case 3:
					pt = ProxyTypes.Socks5;
					break;
			}
			...
			switch(ConnectionType)
			{
				case ConnectionType.Pageant:
					...
					if(pt == ProxyTypes.None)
						...
						info = new AgentConnectionInfo(Host, Port, Username, pt, Proxy, ProxyPort, ProxyUser, ProxyPass,
							agent);
					else
				...
				case ConnectionType.PrivateKey:
					if(pt == ProxyTypes.None)
						info = new PrivateKeyConnectionInfo(Host, Port, Username,
							new PrivateKeyFile(PrivateKey, Passphrase));
					else if(ProxyUser.Length > 0)
						info = new PrivateKeyConnectionInfo(Host, Port, Username, pt, Proxy, ProxyPort, ProxyUser,
							ProxyPass, new PrivateKeyFile(PrivateKey, Passphrase));
					else
						info = new PrivateKeyConnectionInfo(Host, Port, Username, pt, Proxy, ProxyPort,
							new PrivateKeyFile(PrivateKey, Passphrase));
					break;
				default:
					if(pt == ProxyTypes.None)
						...
						info = new PasswordConnectionInfo(Host, Username, Password, pt, Proxy, ProxyPort, ProxyUser,
							ProxyPass);
					else
			...
			_connection = Settings.Default.UseNetworkDrive
				? $"\\\\{info.Host}\\{Root}\\{info.Username}"
				: Name;
			...
			if(Letter != ' ')
			{
				...
				var mountEvent = Task.Factory.StartNew(() =>
				{
					while(!_mountCancel.IsCancellationRequested &&
					      Directory.GetLogicalDrives().All(drive => drive[0] != Letter))
						Thread.Sleep(200);
				}, _mountCancel.Token);
